...
<div class="container-fluid px-4 py-3 pdh-page">
    <div class="pdh-page__header">
        <div>
            <h3 class="mb-1">NUEVO</h3>
            <div class="pdh-page__subtitle">Módulo Compras - Requerimiento de Compras</div>
        </div>
    </div>
    ...
    <?php if (!$hasCentrosAsignados): ?>
        <div class="alert alert-warning" role="alert">
            No tiene centros de costo asignados en `usuarioscentroscosto`. Solicite a Administración la asignación antes de crear un REQ.
        </div>
    <?php endif; ?>
    ...
    <form
        id="req-create-form"
        method="POST"
        action="?route=compras-req/crear"
    ...
    >
        ...
        <div class="compras-req-form__actions d-flex flex-wrap gap-2 justify-content-between align-items-center">
            <div class="small text-muted">Complete cabecera, detalle y firmantes manuales si corresponde.</div>
            <div class="d-flex flex-wrap gap-2">
                <button type="button" class="btn btn-outline-secondary btn-sm" data-req-confirm="cancel-create">
                    <i class="bi bi-x-circle"></i> Cancelar
                </button>
                <button type="button" class="btn btn-outline-primary btn-sm" data-req-submit="guardar_borrador" <?= $hasCentrosAsignados ? '' : 'disabled' ?>>
                    <i class="bi bi-save"></i> Guardar como borrador
                </button>
                <button type="button" class="btn btn-primary btn-sm" data-req-submit="enviar_aprobacion" <?= $hasCentrosAsignados ? '' : 'disabled' ?>>
        ...
        <div class="row g-3">
            ...
            <div class="col-12 col-xl-8">
                <div class="pdh-card p-3 mb-3">
                    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                        <div>
                            ...
                            <div class="compras-req-table-note">No se aceptan cantidades menores o iguales a cero ni items con precio cero.</div>
                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm align-middle req-detail-table" id="req-detalle-table">
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                <?php foreach (($formData['detalle'] ?? []) as $index => $row): ?>
                                    ...
                                    $price = (float)($itemInfo['invitemcostoestandar'] ?? 0);
                                    $qty = (float)str_replace(',', '.', (string)($row['reqcompradetcantidad'] ?? '0'));
                                    $itemCode = (string)($itemInfo['erpinvitemcod'] ?? $row['invitemid'] ?? '');
                                    $itemDesc = (string)($itemInfo['invitemdsc'] ?? $row['invitemid'] ?? '');
                                    $unitName = (string)($itemInfo['invunidmeddsc'] ?? '');
                                    $subfamilyName = (string)($itemInfo['subfamiliadsc'] ?? '');
                                    $subfamiliaId = (string)($itemInfo['subfamiliaid'] ?? '');
                                    ?>
                                    <tr
                                        data-item-id="<?= htmlspecialchars((string)($row['invitemid'] ?? '')) ?>"
                                        data-item-price="<?= htmlspecialchars((string)$price) ?>"
                                        data-subfamily-id="<?= htmlspecialchars($subfamiliaId) ?>"
                                        data-subfamily-name="<?= htmlspecialchars($subfamilyName) ?>"
                                    >
                                        <td class="text-end">
                                            <button class="btn btn-outline-danger btn-sm req-remove-row" type="button" title="Eliminar item" aria-label="Eliminar item">
                                                <i class="bi bi-trash"></i>
                                            </button>
                                        </td>
                                        <td>
                                            <input type="hidden" name="detalle[<?= $index ?>][invitemid]" value="<?= htmlspecialchars((string)($row['invitemid'] ?? '')) ?>">
                                            <div class="compras-req-item-cell__title"><?= htmlspecialchars($itemDesc) ?></div>
                                        ...
                                        <td>
                                            <input class="form-control req-detalle-cantidad text-end" type="number" min="0.0001" step="0.0001" name="detalle[<?= $index ?>][reqcompradetcantidad]" value="<?= htmlspecialchars((string)($row['reqcompradetcantidad'] ?? '')) ?>">
                                        </td>
                                        <td class="text-end"><span class="req-detalle-price"><?= htmlspecialchars(number_format($price, 0, ',', '.')) ?></span></td>
                                        <td class="text-end"><span class="req-detalle-total"><?= htmlspecialchars(number_format($price * $qty, 0, ',', '.')) ?></span></td>
                    ...
                    <div id="req-detalle-empty" class="compras-req-empty <?= !empty($formData['detalle']) ? 'd-none' : '' ?>">Todavía no hay items agregados.</div>
                </div>
                ...
                <div class="pdh-card p-3">
                    ...
                    <div class="compras-req-table-note mb-3">Disponible por subfamilia según los ítems hoy seleccionados. El cálculo formal se ejecutará al persistir el REQ.</div>
                    <div class="table-responsive">
                    ...
                    <div id="req-live-ppto-empty" class="compras-req-empty">El análisis informativo se actualizará con los ítems del detalle.</div>
                </div>
            </div>
        </div>
    </form>
                            ...
                            <?php foreach ($itemsRows as $item): ?>
                                ...
                                >
                                    <td class="text-center">
                                        <button type="button" class="btn btn-outline-primary btn-sm req-add-item" title="Agregar item" aria-label="Agregar item">
                        ...
                        <tbody>
                            <?php foreach ($aprobadoresRows as $aprobador): ?>
                                ...
                                >
                                    ...
                                    <td class="text-end">
                                        <button type="button" class="btn btn-outline-primary btn-sm req-add-firmante"><i class="bi bi-person-plus"></i></button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const form = document.getElementById('req-create-form');
        ...
        document.querySelectorAll('[data-req-submit]').forEach((button) => {
            button.addEventListener('click', function () {
                ...
                confirmMessageEl.textContent = action === 'guardar_borrador'
                    ? 'Se guardará el REQ como borrador. ¿Desea continuar?'
                    : 'El REQ se enviará a aprobación. ¿Desea continuar?';
                confirmAction = () => {
        ...
        document.querySelector('[data-req-confirm="cancel-create"]')?.addEventListener('click', function () {
            confirmMessageEl.textContent = 'Esta acción borrará los datos ingresados. ¿Desea continuar?';
            confirmAction = () => { window.location.href = '?route=compras-req/listar'; };
            confirmModal?.show();
        });
